...
until kubectl wait --for=condition=available --timeout=600s deployment/storageos-api-manager -n kube-system >/dev/null; do sleep 1; done
echo ✅

printf "⏳ ${CYAN}Waiting for storageos-csi-helper  - ${NC}"
until kubectl wait --for=condition=available --timeout=600s deployment/storageos-csi-helper -n kube-system >/dev/null; do sleep 1; done
echo ✅

printf "⏳ ${CYAN}Waiting for storageos-scheduler   - ${NC}"
until kubectl wait --for=condition=available --timeout=600s deployment/storageos-scheduler -n kube-system >/dev/null; do sleep 1; done
echo ✅
